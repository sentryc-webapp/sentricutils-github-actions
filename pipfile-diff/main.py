...
def parse_pipfile_lock() -> Dict:
    ...
    """
    deps = {}
    with open(os.environ["GITHUB_WORKSPACE"] + "/Pipfile.lock") as f:
...
def get_changed_deps(base_deps, head_deps) -> Dict:
    ...
    for name, version in base_deps.items():
        if name in head_deps and version != head_deps[name]:
            data[name] = {"base": version, "head": head_deps[name]}
    return data
...
def get_issue_info():
    ...
    g = Github(os.environ["INPUT_REPO_TOKEN"])
...
def create_comment(message: str) -> None:
    ...
    # Check for existing comment
    existing_issue_id, pull = get_issue_info()
...
def run():
    ...
    subprocess.run(
        ["git", "fetch", "origin", os.environ["GITHUB_BASE_REF"], "--no-tags", "--prune", "--unshallow"],
    ...
    if head_deps == base_deps:
        print("No dependency changes. Deleting comment if it existed.")
        delete_comment()
        return
